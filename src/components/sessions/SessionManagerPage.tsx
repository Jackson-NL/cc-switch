...
export function SessionManagerPage({ appId }: { appId: string }) {
  ...
  const sessionMetaMutation = useMutation({
    mutationFn: sessionsApi.updateMeta,
    onSuccess: async () => {
  ...
  const openRenameDialog = (session: SessionMeta) => {
    ...
    setRenameValue(session.customTitle ?? formatSessionTitle(session));
  };
  ...
  const confirmRename = async () => {
    ...
    try {
      await sessionMetaMutation.mutateAsync({
        ...
        customTitle: renameValue.trim() || null,
      });
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      await Promise.allSettled(
        results
          .filter((result) => result.success && result.sourcePath)
          .map((result) =>
            sessionsApi.clearMeta({
  ...
  return (
    <TooltipProvider>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {selectedSession.isPinned && (
                            <Pin className="size-4 shrink-0 text-amber-600" />
                          )}
                        </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open && !sessionMetaMutation.isPending) {
            setRenameTarget(null);
          }
        }}
      >
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button
              ...
              disabled={sessionMetaMutation.isPending}
            >
